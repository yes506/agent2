...
def summarize_travel_flows(travel_flows: dict) -> str:
    ...
    summary_agent = AssistantAgent(
        ...
        code_execution_config=False,
        system_message="""
        # 역할: 너는 여행 기록을 요약하여 기록하는 전문가야.
        
        # 여행 요약 및 기록 시 필수 고려 사항:
            - 요약 기록 생성을 절대 건너 뛰거나 누락하지 마.
            - 전달된 여행 정보를 기반으로 여행을 분석하고 기록을 생성해.
            - 400자 이내로 작성해.
            - 소설가 처럼 이야기 하듯이 작성해.
            - 반드시 현재 시제로 표현해. 과거 시제 사용 금지.
            - 너의 문장 구성 능력을 최대한 발휘하여 작성해.
            - 요약글 작성이 완료되면 반드시 SayTerminate 도구를 실행하여 대화를 종료해.
            - 필수 포함 항목: 여행 날짜(연도-월-일), 여행 멤버, 여행 멤버의 관계, 여행 목적, 여행 장소 목록.
            - 요약 예시 : 2024년 8월 2일 여행 멤버 남성0001, 여성0001, 여성0002 3명이 여름휴가 목적으로 여행한다. 장소는 서울 광화문, 경복궁, 맥도날드...(더 상세하게)
